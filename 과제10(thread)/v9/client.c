...
int main() {
    ...
    while (1) {
        ...
        while (1) {
            ...
            // 서버로부터 시간 데이터 수신
            ssize_t time_len = read(sock, &received_time, sizeof(received_time));
            if (time_len <= 0) {
                printf("Error receiving time from server.\n");
                break;
            }
            ...
            // 서버로부터 IP 정보 수신
            ssize_t ip_len = read(sock, &server_info, sizeof(server_info));
            if (ip_len <= 0) {
                printf("Error receiving IP info from server.\n");
                break;
            }
